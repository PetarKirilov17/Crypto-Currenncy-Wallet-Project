...
public class CommandValidator {
    ...
    private static final String WRONG_USERNAME_MESSAGE =
        "Username has to be between " + StringValidator.MIN_USERNAME_LENGTH + " and " +
            StringValidator.MAX_USERNAME_LENGTH + " characters long! It must contain only alphanumeric characters!";
    private static final String WRONG_PASSWORD_MESSAGE =
        "Password has to be between " + StringValidator.MIN_PASSWORD_LENGTH + " and " +
            StringValidator.MAX_PASSWORD_LENGTH;
    private static final String WRONG_DOUBLE_NUMBER_MESSAGE =
        "Double number is not in correct format! Please insert a valid number!";
    private static final String WRONG_POSITIVE_INTEGER_MESSAGE =
        "Page number is not in correct format! Please insert a valid number!";
    ...
    private boolean checkSizeOfPassedArguments(int argsCount, int desiredCount) {
        if (argsCount != desiredCount) {
            ui.writeError(INVALID_SIZE_OF_ARGS + " Desired count: " + desiredCount + " | Count of passed arguments: " +
                argsCount);
            return false;
        }
        return true;
